...
<div class="row">
    <div class="col-xl-12 col-xxl-12 col-sm-12">
        <div class="card">
            ...
            <div class="card-body">
                <p class="text-body mb-4">Oversee instructor–batch assignments. Assign, swap, or remove instructors without affecting student data.</p>
                <form method="get" action="{{ route('super-admin.batch-management.index') }}" class="row g-2 align-items-end mb-4">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-3">
                        <label for="active" class="form-label small mb-0">Status</label>
                        <select name="active" id="active" class="form-select form-select-sm" onchange="this.form.submit()">
                    ...
                    <div class="col-auto">
                        <a href="{{ route('super-admin.batch-management.index') }}" class="btn btn-sm btn-outline-secondary">Reset</a>
                    </div>
                </form>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        @if($batch->instructor_id)
                                            ...
                                            <form method="post" action="{{ route('super-admin.batch-management.remove', $batch) }}" class="d-inline" onsubmit="return confirm('Remove this instructor from the batch? The batch will become vacant.');">
                                                @csrf
...
@foreach($batches as $batch)
    ...
    <div class="modal fade" id="modalAssign{{ $batch->id }}" tabindex="-1" aria-labelledby="modalAssignLabel{{ $batch->id }}" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="{{ route('super-admin.batch-management.assign', $batch) }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <select name="instructor_id" id="assign_instructor_{{ $batch->id }}" class="form-select" required>
    ...
    <div class="modal fade" id="modalSwap{{ $batch->id }}" tabindex="-1" aria-labelledby="modalSwapLabel{{ $batch->id }}" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="{{ route('super-admin.batch-management.swap', $batch) }}">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <select name="instructor_id" id="swap_instructor_{{ $batch->id }}" class="form-select" required>
                            <option value="">Select instructor</option>
                            @foreach($instructors as $inst)
