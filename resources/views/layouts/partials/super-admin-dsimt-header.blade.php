{{-- Super Admin header (search, theme, notifications, profile) --}}
<div class="header">
                    ...
                    <li class="nav-item dropdown notification_dropdown">
                        <a class="nav-link bell ai-icon" href="javascript:void(0);" role="button" data-bs-toggle="dropdown">
                            ...
                            @if(isset($notifications) && count($notifications ?? []) > 0)
                                <span class="badge badge-sm badge-danger rounded-circle pulse-css">!</span>
                            @endif
                        </a>
                        <div class="dropdown-menu dropdown-menu-end">
                                ...
                                @forelse(($notifications ?? []) as $notif)
                                    <li class="media dropdown-item align-items-center gap-3">
                                        ...
                                        <div class="media-body">
                                            <a href="{{ $notif['url'] ?? '#' }}"><p class="mb-0"><strong>{{ $notif['title'] ?? 'Notification' }}</strong> {{ $notif['message'] ?? '' }}</p></a>
                                        </div>
                                    </li>
                                @empty
                            ...
                            @if(isset($notifications) && count($notifications) > 0)
                                <a class="all-notification d-block text-center py-2" href="{{ route('super-admin.registrations.index') }}">See all <i class="ti-arrow-right"></i></a>
                            @endif
                        </div>
                    </li>
