                        ...
                        <tbody>
                            @forelse($pendingRegistrations as $u)
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" action="{{ route('admin.registrations.approve', $u) }}" class="d-inline">@csrf<button type="submit" class="btn btn-sm btn-success">Approve</button></form>
                                        <form method="post" action="{{ route('admin.registrations.reject', $u) }}" class="d-inline" onsubmit="return confirm('Reject and remove this registration?');">@csrf<button type="submit" class="btn btn-sm btn-outline-danger">Reject</button></form>
                                    </td>
                                </tr>
                            @empty
